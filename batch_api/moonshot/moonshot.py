"""Batch generation via OpenAI Batch API; output matches main.py evaluation format."""
...
from common_config import (
    ...
    read_existing_generated_hash_keys_simpleqa,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Pipeline step. Full flow: prepare / upload / create / wait / collect. Use cancel to cancel a batch.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--input-csv",
        type=str,
        default="dataset/FACTS-Parametric-public.csv",
        help="FACTS CSV path.",
    )
    parser.add_argument(
        "--save-to",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="If set, only the first k rows of the CSV are considered.",
    )
    parser.add_argument(
...
def _default_output_path(model_id: str, input_csv: str) -> str:
...
def _resolve_responses_path(args: argparse.Namespace) -> str:
    if args.save_to:
        ...
        ensure_parent_dir(p)
        return p
    p = _default_output_path(str(args.model_id), _input_csv_path(args))
    ensure_parent_dir(p)
    return p
...
def save_json(path: Path, data: Any) -> None:
    ...
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        json.dump(order_result_payload(data), f, ensure_ascii=False, indent=2)
...
def build_batch_input_file_simpleqa(
    ...
    with open(input_path, "w", encoding="utf-8", newline="\n") as f:
        for key, payload, messages in items:
            ...
            key_payloads[sk] = {
                "key": int(key),
                "hash_key": str(payload.get("hash_key", "")),
...
def stage_prepare(args: argparse.Namespace) -> Path:
    ...
    existing_hash_keys = read_existing_generated_hash_keys_simpleqa(responses_output)
    resume_skipped = sum(1 for row in dataset if row["hash_key"] in existing_hash_keys)
    if existing_hash_keys:
        print(
            f"resume: {len(existing_hash_keys)} hash_key(s) already in {responses_output} will be skipped"
        )
    for key, row in enumerate(dataset):
        all_keys.add(int(key))
        if row["hash_key"] in existing_hash_keys:
            continue
    ...
    metadata: dict[str, Any] = {
        "version": 3,
        "task_type": "FACTS-Parametric-public",
        "model": model_id,
...
def stage_cancel(args: argparse.Namespace, run_dir: Path | None) -> Any:
    ...
    if run_dir is not None:
        ...
        batch_id = args.batch_id or ch.get("batch_id")
        if not batch_id:
            raise ValueError("batch_id missing; use create first, --batch-id, or a run-dir with batch_id in meta")
    else:
        chunk_i = None
        model_id = str(args.model_id)
        batch_id = args.batch_id
        if not batch_id:
            raise ValueError("cancel requires --batch-id when --run-dir is not provided")

    client = make_client(model_id, _models_yaml_path(args))
    batch = client.batches.cancel(str(batch_id))

    if metadata is not None and meta_path is not None and ch is not None:
        ch["batch_id"] = str(batch_id)
        ch["batch_status"] = batch.status
        save_json(meta_path, metadata)
        print(f"cancel requested, chunk={chunk_i}, batch_id={batch_id}, status={batch.status}")
    else:
        print(f"cancel requested, batch_id={batch_id}, status={batch.status}")
    return batch
...
def stage_collect(
    ...
    for sk, res in key_results.items():
        ...
        if not isinstance(text, str) or text.strip() == "":
            ...
            meta_row = key_payloads.get(sk, {})
            hash_key = build_hash_key_from_result_row(meta_row) if isinstance(meta_row, dict) else ""
            print(f"skip empty llm_answer from batch output: key={sk}, hash_key={hash_key}")
            continue
        meta_row = key_payloads[sk]
        hash_key = build_hash_key_from_result_row(meta_row)
        to_write.append(
            {
                "hash_key": str(hash_key or ""),
    ...
    if to_write:
        ...
        if not isinstance(existing, list):
            existing = []
        payload["model_config"] = metadata.get("output_model_config", {})
        payload["results"] = upsert_simpleqa_results(existing, to_write)
        save_json(Path(results_path), payload)

    done = set(metadata.get("completed_chunk_indices", []))
    done.add(chunk_i)
    metadata["completed_chunk_indices"] = sorted(done)
    metadata["success_keys_last_chunk"] = sorted(success_keys)
    ...
    print(f"evaluate next: python main.py --evaluate-file \"{results_path}\" --judge-model deepseek-chat")
    print(
...
def main() -> None:
    ...
    if step == "cancel" and not args.run_dir and not args.batch_id:
        raise SystemExit("--step cancel requires --run-dir or --batch-id")
    ...
    if step == "collect":
        ...
        return
    if step == "cancel":
        run_dir = _run_dir_from_arg(args.run_dir) if args.run_dir else None
        stage_cancel(args, run_dir)
        return
