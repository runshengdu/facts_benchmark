"""FACTS batch generation for Volcengine Ark (Doubao) via Batch Job + TOS.
...
from common_config import load_yaml_config, save_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="FACTS: Doubao batch inference (Ark Job API + TOS)."
    )
    add_batch_arguments(
        parser,
...
def stage_prepare(args: argparse.Namespace) -> Path:
    ...
    return prepare_batch_run(
        ...
        provider=provider,
        defaults=defaults,
        build_chunk_input=build_batch_input_file_job,
...
def stage_create(args: argparse.Namespace, run_dir: Path) -> str:
    ...
    job_id = create_batch_inference_job(
        make_ark_api(),
        name=f"facts-{model_id}-c{chunk_i:03d}-{run_dir.name}"[:128],
        ...
        completion_window=completion_window,
        project_name=ARK_PROJECT_NAME,
        description=f"facts chunk {chunk_i}",
    )
