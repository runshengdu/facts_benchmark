"""FACTS batch generation via OpenAI-compatible Batch API (chat completions).
...
from common_config import (
    ...
    parse_batch_output,
    read_existing_generated_hash_keys,
    sanitize_path_component,
    save_json,
    save_text,
    strip_sensitive_config,
    upsert_generation_results,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="FACTS: generate with Batch API; evaluate with main.py --evaluate-file."
    )
    add_batch_arguments(
        parser,
...
def prepare_batch_run(
    ...
    *,
    ...
    items, resume_skipped = iter_batch_items(args, responses_output)
    if not items:
    ...
    metadata: dict[str, Any] = {
        ...
        "task_type": "facts_parametric_public",
        "provider": provider,
        "model": model_id,
...
def finish_batch_collect(
    ...
    print(
        f"evaluate next: python main.py --evaluate-file \"{results_path}\" "
        f"--judge-model deepseek-v4-pro"
    )
    print(
...
def iter_batch_items(
    ...
    dataset = load_facts_dataset(_input_csv_path(args), args.num_tasks)
    existing_hash_keys = read_existing_generated_hash_keys(responses_output)
...
def build_batch_input_file_job(
    items: list[tuple[int, dict[str, Any], list[dict[str, Any]]]],
    model_cfg: dict[str, Any],
    input_path: Path,
) -> dict[str, dict[str, Any]]:
...
def apply_collect_results(
    ...
    for sk, res in key_results.items():
        ...
        to_write.append(
            {
                "hash_key": str(meta_row.get("hash_key", "")),
    ...
    results_path = str(metadata["save_to"])
    if to_write:
        payload = load_generation_output(results_path)
        existing = payload.get("results", [])
        if not isinstance(existing, list):
            existing = []
        merged_results = upsert_generation_results(existing, to_write)
        model_config = metadata.get("output_model_config", {})
        ordered_payload = build_generation_output_payload(
...
def build_batch_input_file_openai(
    items: list[tuple[int, dict[str, Any], list[dict[str, Any]]]],
    model_id: str,
    model_cfg: dict[str, Any],
    input_path: Path,
...
def stage_prepare(args: argparse.Namespace) -> Path:
    ...
    def build_chunk_input(
        ...
        return build_batch_input_file_openai(
            items=chunk_items,
            model_id=model_id,
            model_cfg=model_cfg,
...
def main() -> None:
    ...
    run_batch_pipeline(
        ...
        all_chunks_done_message="all batch chunks for this run finished (see generation JSON).",
    )
